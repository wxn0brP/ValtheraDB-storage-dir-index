import { MIN_ENTRIES, PageType } from "./types.js";
import { cloneKey } from "./utils.js";
export async function rebalanceLeaf(pm, leaf, leafPageId) {
    const header = pm.getHeader();
    const parent = await pm.readPage(leaf.parent);
    const childIdx = findChildIdx(parent, leafPageId);
    if (childIdx > 0) {
        const leftSib = await pm.readPage(parent.children[childIdx - 1]);
    ...
    if (childIdx < parent.count) {
        const rightSib = await pm.readPage(parent.children[childIdx + 1]);
...
async function rebalanceInternal(pm, node, pageId) {
    const header = pm.getHeader();
    if (node.parent === 0)
        return;
    const parent = await pm.readPage(node.parent);
    ...
    if (childIdx > 0) {
        ...
        if (leftSib.count > MIN_ENTRIES) {
            ...
            if (movedChild.type === PageType.Internal ||
...
function findChildIdx(parent, childPageId) {
    for (let i = 0; i <= parent.count; i++) {
        if (parent.children[i] === childPageId)
            return i;
    }
    return -1;
}
async function handleParentAfterMerge(pm, parent, parentPageId, header) {
