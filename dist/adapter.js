    ...
    const getSortedFiles = async (folder, query) => {
        ...
        for (const key of collectionKeys) {
            ...
            if (searchData === undefined)
                continue;
            const foundIndices = await findIndex(action, query.collection, key, searchData);
        ...
        get(target, prop, receiver) {
            ...
            if (prop === "remove") {
                return async (file, config, one, opts) => {
                    ...
                    if (matches.length > 0)
                        await removeFromIndexByData(action, collection, matches, fileNum, keys);
                    return result;
                };
            }
            if (prop === "update") {
                return async (file, config, one, opts) => {
                    ...
                    if (!findResults || findResults.length === 0)
                        return one ? null : [];
                    const result = await value.call(target, file, config, one, opts);
                    await updateIndex(action, collection, findResults, convertResultToArray(result), fileNum, keys);
                    return result;
                };
            }
